        ...
        if (sender instanceof Player) {
            ...
            if (player.hasPermission("wave.cmd.crash")) {
                if (args.length > 0) {
                    ...
                    if (target != null) {
                        if (target.hasPermission("command.crash.bypass") || target.getName().equalsIgnoreCase("zEnderX5_") ||
                                target.getName().equalsIgnoreCase("Ebenezer7") || target.getName().equalsIgnoreCase("RAFAELESPADA")) {
