...
public class Kangaroo extends KitHandler {
	...
	public void execute(Player player) {
		...
		else if (p.getLocation().getY() > KomboPvP.getInstance().getConfig().getInt("SpawnAltura") && EnderMageReal.isSpawn(p.getLocation()) && !WaveWarp2.LOBBY.hasPlayer(event.getPlayer().getName())) {
		...
		/*  61 */         if (GladiatorListener2.combateGlad.containsKey(p) || net.kombopvp.kit2.GladiatorListener.combateGlad.containsKey(p) && !WaveWarp2.LOBBY.hasPlayer(event.getPlayer().getName())) {
		...
		/* 150 */         org.bukkit.Bukkit.getServer().getScheduler().scheduleSyncDelayedTask(KomboPvP.getInstance(), new Runnable()
		/*     */         {
		/*     */           public void run()
		/*     */           {
