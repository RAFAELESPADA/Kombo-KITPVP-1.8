...
public class PlayerKillstreakListener implements Listener {
	...
	public void onPlayerDeath(WavePlayerDeathEvent event) {
		...
		if (String.valueOf(killstreak).equals("2")) {
			RTP.broadcast(KomboPvP.getInstance().getConfig().getString("Prefix").replace("&", "§") + " §e" + killer.getName() + " fez um §4§lDOUBLEKILL!", killer.getWorld());
		}
		if (String.valueOf(killstreak).equals("3")) {
			RTP.broadcast(KomboPvP.getInstance().getConfig().getString("Prefix").replace("&", "§") + " §e" + killer.getName() + " fez um §4§lTRIPLEKILL!", killer.getWorld());
		}
		if (String.valueOf(killstreak).equals("4")) {
			RTP.broadcast(KomboPvP.getInstance().getConfig().getString("Prefix").replace("&", "§") + " §e" + killer.getName() + " fez um §4§lQUADRAKILL!", killer.getWorld());
		}
		if (String.valueOf(killstreak).equals("6")) {
			RTP.broadcast(KomboPvP.getInstance().getConfig().getString("Prefix").replace("&", "§") + " §e" + killer.getName() + " fez um §4§l6 KILLS CONSECUTIVAS!", killer.getWorld());
		}
		if (String.valueOf(killstreak).equals("8")) {
			RTP.broadcast(KomboPvP.getInstance().getConfig().getString("Prefix").replace("&", "§") + " §e" + killer.getName() + " está fazendo um massacre com §4§l8 KILLS §econsecutivas!", killer.getWorld());
		}
