             ...
             if (WaveWarp2.SPAWN.hasPlayer(winner.getName())) {
                 ...
                 player.getInventory().setItem(0, (new ItemBuilder("§aKits 1", Material.CHEST))
             	   ...
             	   WaveBukkit.getInstance().getPlayerManager().getController().save(k);
         			}}.runTaskLater(KomboPvP.getInstance(), 7);
              return;
                 }
         }
		 if (net.kombopvp.kit2.GladiatorListener.combateGlad.containsKey(player)) {
             final Player winner = net.kombopvp.kit2.GladiatorListener.combateGlad.get(player);
             final Player loser = player;
             net.kombopvp.kit2.GladiatorListener.resetGladiatorListenerByKill(winner, loser);
             net.kombopvp.kit2.GladiatorListener.combateGlad.remove(winner);
             net.kombopvp.kit2.GladiatorListener.combateGlad.remove(loser);
             if (WaveWarp2.SPAWN.hasPlayer(winner.getName())) {
             WaveWarp2.SPAWN.send(loser);
             new BukkitRunnable() {
      			@Override
...
player.updateInventory();
 ...
 WaveBukkit.getInstance().getPlayerManager().getController().save(k);
	...
	if (!KitManager.getPlayer(event.getPlayer().getName()).hasKit() && event.getPlayer().getLocation().getY() > KomboPvP.getInstance().getConfig().getInt("SpawnAltura") && EnderMageReal.isSpawn(event.getPlayer().getLocation())) {
			       ...
			       EventoUtils.getEventoPlayers().forEach(p -> {
			    	   Bukkit.broadcastMessage("§6GANHADOR DO EVENTO §a§ §C§l " + EventoUtils.getEventoPlayersNames());
		            	...
		            	p.setHealth(20);
		            	      ...
		            	      p.setFlying(false);
		            	      KomboPvP.getInstance().getEventManager().getRdmAutomatic().setGameType(RDMAutomatic.GameType.STOPPED);
		            	      ...
		            	      RDMAutomatic.star = false;
	            	    	     ...
	            	    	     WavePlayer player = WaveBukkit.getInstance().getPlayerManager().getPlayer(p.getName());
	            	    	      ...
	            	    	      KomboPvP.getInstance().getEventManager().getRdmAutomatic().players.remove(p);
	            	    		  WaveBukkit.getInstance().getPlayerManager().getController().save(player);
		            	      EventoUtils.resetEventoClass();
		            	    		Bukkit.broadcastMessage("§6O Evento 1V1 foi finalizado!");
		            	    		 KomboPvP.getInstance().getEventManager().getRdmAutomatic().removeFromEvent(p);
		            	      p.setAllowFlight(false);
		            	      
		            	      Bukkit.getConsoleSender().sendMessage("PARANDO EVENTO 1V1");
		            	    
		            	   KomboPvP.getInstance().getEventManager().setRdmAutomatic(null);
			       });
			       };
			       @EventHandler
