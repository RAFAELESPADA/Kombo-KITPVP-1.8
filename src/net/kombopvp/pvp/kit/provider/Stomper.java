...
public class Stomper extends KitHandler {
	...
	public void onDamage(EntityDamageEvent event) {
		...
		if (mage.getLocation().getY() > KomboPvP.getInstance().getConfig().getInt("SpawnAltura") && EnderMageReal.isSpawn(mage.getLocation())) {
			return;
		 }
			   ...
			   if (!(e.getEntity() instanceof Player)) {
				...
				else if(e.getCause() == EntityDamageEvent.DamageCause.ENTITY_ATTACK)
				{
					this.fall.add(p.getName());
					 Bukkit.getScheduler().scheduleSyncDelayedTask(KomboPvP.getInstance() , new BukkitRunnable() {
				         @Override
				         public void run() {
