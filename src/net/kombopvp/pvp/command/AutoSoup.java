...
public class AutoSoup implements CommandExecutor, Listener {
  ...
  public AutoSoup(KomboPvP main) {}
  
  public boolean onCommand(CommandSender sender, Command cmd, String Label, String[] args) {
    Player p = (Player)sender;
    if (cmd.getName().equalsIgnoreCase("autosoup")) {
      if (!p.hasPermission("wave.cmd.autosoup")) {
      ...
      Bukkit.getServer().getScheduler().scheduleSyncDelayedTask((Plugin)KomboPvP.getInstance(), new Runnable() {
            public void run() {
