...
public class Duelos implements Listener {
	...
	public static void open(Player player, Player target) {
   	...
   	PlayerPvP pvp = helixPlayer.getPvp();
		...
		inventory.setItem(12, new ItemBuilder("§cSoup 1v1", Material.MUSHROOM_SOUP).nbt("swertgh").lore("§fWins na 1V1: §a" + pvp.getWinsx1() ,"§fPerdas na 1V1: §a" + pvp.getDeathsx1() ,"§fWinStreak na 1V1: §a" + pvp.getWinstreakx1())
				.toStack()
		);
		inventory.setItem(14, new ItemBuilder("§cSumô", Material.LEASH).nbt("swertgh").lore("§fWins no Sumô: §a" + pvp.getWinssumo() ,"§fPerdas no Sumô: §a" + pvp.getDeathssumo() ,"§fWinStreak na Sumô: §a" + pvp.getWinstreaksumo()).toStack());
